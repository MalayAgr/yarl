...
class GameWorld:
    ...
    ENEMIES_FLOOR_COUNTS: list[tuple[int, int]] = [(1, 2), (4, 3), (6, 5)]
    ...
    ITEMS_FLOOR_COUNTS: list[tuple[int, int]] = [(1, 1), (4, 2)]
    """Maximum number of items per room by floor.
    ...
    ENEMIES_FLOOR_FACTORIES: dict[int, dict[ActiveEntity, float]] = {
        0: {ENEMIES["orc"]: 0.43},
    ...
    ITEM_FLOOR_FACTORIES: dict[int, dict[Item, float]] = {
        0: {
    ...
    def generate_floor(self, player: ActiveEntity | None = None) -> GameMap:
        ...
        enemy_factory = self.get_factory_by_floor(
            floor_factories=self.ENEMIES_FLOOR_COUNTS
        )
